...
sub write {
	...
	if ($self->{__append__} == 1) {
		foreach my $key (keys %{$self}) {
			...
			if (!exists $used->{$key}) {
				print "Block doesn't exist: $block!\n" if $self->{__debug__} == 1;
				push (@new, "");
				push (@new, "[$key]");
			}
...
B<__debug__> - Set to true and you will see debugging messages. Defaults to 0.
